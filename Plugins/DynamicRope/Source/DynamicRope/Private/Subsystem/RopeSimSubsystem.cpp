...
void URopeSimSubsystem::Tick(float DeltaTime)
{
	...
	if (CVarRopeGPUSolver.GetValueOnGameThread() != 0)
	{
		...
		for (URopeComponent* Rope : Ropes)
		{
			...
			// 충돌(M2): 이 로프의 collider 중 capsule만 추출(SDF 등 비-capsule은 GetGPUCapsule이 false → 제외).
			...
			{
				FRopeGPUCapsule Cap;
				if (Collider && Collider->GetGPUCapsule(Cap.A, Cap.B, Cap.Radius))
				{
					AllCaps.Add(Cap);
				}
			}
		...
		if (Jobs.Num() > 0)
		{
			// AllCaps 완성 후 포인터 fixup(앞서 Add 중 재할당으로 무효화될 수 있어 여기서 한다).
