...
//
// 데이터는 전부 URopeComponent의 public 게터(GetPhase/GetNodeCount/IsSleeping/GetSolverLODScale/
// IsGpuSteppedThisFrame/WasSolvedThisFrame/bUseWorldGDF/GetCenterlinePositions)에서 라이브로 읽는다 —
// 서브시스템/스냅샷 의존 없음. 로프 목록은 월드 순회(TObjectIterator + IsRegistered + world 필터)로 잡아
// 서브시스템 private Ropes 배열을 건드리지 않는다. 카메라 거리는 OwnerPC 카메라에서 직접 산출.
