...
// does nothing. Each frame, for every wrapping rope, three gates must pass before the handle swings:
...
//  3) the pull direction, from the handle tip towards the rope's hand end, actually points along the
//     tip's swing arc — the alignment is the dot product against the arc tangent, so pulling square
//     along the arc swings fastest, pulling at a grazing angle swings slowly, and pulling from the
//     wrong side does nothing.
// The swing rate is scaled by that alignment, which reads as the handle following the player's pull
...
// button. Holding the prop up therefore means holding the pull, unless bHoldAtEnd locks the handle
// at the pulled end, which turns a completed pull into a persistent state that only SetOn(false)
// releases. Independently of that, reaching the end of the travel still toggles the logical On/Off
...
public:
	...
	/** Minimum alignment between the pull direction and the handle tip's swing arc, as a dot product
	 *  in -1 to 1. Below it the pull is treated as sideways or from the wrong side and ignored. */
	...
	/** Lock the handle at the pulled end once a pull completes, instead of springing back, so the
	 *  target stays fully rotated with no need to keep holding the pull. The lever becomes a
	 *  one-way switch: only SetOn(false), from Blueprint or the details panel button, releases the
	 *  hold and lets the handle spring home. */
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Rope|Demo|Pull")
	bool bHoldAtEnd = false;

	/** Draw the pull gate every frame: the handle tip, the swing arc tangent in yellow, and each
	 *  wrapping rope's pull direction, green when it passes and red when a gate rejects it, with the
	 *  per-gate values printed at the tip. The fastest way to see why a pull is not moving the
...
private:
	...
	/** The strongest swing rate any wrapping rope produces this frame (deg/s), 0 when none of them
	 *  passes the taut, tension and alignment gates. */
	float ComputePullRate() const;
