...
// 컨벡스 심플 콜리전은 평면 집합 collider(FRopeConvexCollider)로 추출하고(평면 수 상한 초과분은
// OBB 폴백), ISM/HISM 인스턴스도 region 겹침 단위로 지원한다.
...
private:
	 ...
	 * 단 이 provider는 로프 region 목록(서브시스템이 프레임당 동일 값 전달)을 실제로 쓴다.
	 */
	...
	/**
	 * ISM/HISM: region과 겹치는 인스턴스만 열거해 각 인스턴스 월드 트랜스폼으로 공유 BodySetup을 추출한다
