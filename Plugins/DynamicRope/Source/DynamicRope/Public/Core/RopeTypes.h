...
struct FRopeHoldConfig
{
	...
	/**
	 * (두 모드 공용 — 물리 바디 대상 한정) 대상 견인 servo가 당김 방향과 *직교*인 잔여 속도를 프레임당 감쇠하는
	 * 비율(0 = 보존, 1 = 완전 제거). servo는 로프 축 속도만 목표로 맞추고 직교 성분은 그대로 둔다 — 그래서 끌던
	 * 방향을 급전환하면 옛 방향 관성이 직교로 남아 대상이 옆으로/위로 날아간다("관성 과다"). 이 값으로 그 잔여
	 * 관성을 몇 프레임에 걸쳐 빼되(중력/스윙은 매 프레임 재축적돼 대부분 보존), 급전환 fling을 억제한다. 기본 0.3.
	 * CMC 캐릭터에는 적용하지 않는다 — 관성이 없어(매 틱 입력으로 속도 재유도) 남을 잔여 관성 자체가 없다.
	 */
	 ...
	 * 이 장력 안에서만 끌려온다 — 프레임당 ΔV를 J/mass = min(dV, 이 값·dt/mass)로 제한한다. 가벼운 대상은 목표
	 * 리엘 속도(min(TetherReelSpeed, TetherMaxSpeed))에 도달해 잘 따라오고, 무겁거나 접지 마찰이 큰 대상은 장력
	 * 한계로 뒤처지며 그만큼 로프가 자연스럽게 늘어난다("무게감"). 클수록 무거운 대상도 안 늘어나게 따라오고,
	 * 작을수록 무게/마찰이 도드라진다.
	 *
	 * **튜닝의 핵심 — 뒤처짐은 이 값으로만 잡힌다**: 질량 M이 목표 속도 V에 한 프레임 만에 도달하는 문턱 장력이
	 * ≈ M·V·fps라, 50kg을 400cm/s로 스냅하게 끌려면 ≈ 1.2M이 필요하다(기본 150k에서는 프레임당 ΔV가 50cm/s라
	 * 0.13초쯤 뒤처진다). 리엘 속도 상한(TetherReelSpeed/TetherMaxSpeed)을 올려도 뒤처짐은 안 잡힌다 — 장력이
	 * 지배하는 구간이라 도달 속도가 그대로다(CL 401에서 실제로 그 실수를 했다).
	 * 0 = 무제한(질량 무시 → 목표 속도에 즉시 도달 = 정확 서보, 안정). MassShare 모드는 쓰지 않는다(양끝 분배로 처리).
	 */
